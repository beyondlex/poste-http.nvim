...
- 2026-06-27: http — run directive post-script executed twice because `return` after `execute_run_directive()` was removed during edit. Without `return`, execution falls through to normal request path, running the `run` line as a malformed HTTP request. Fix: always preserve `return` after run directive branch. See `lua/poste/http/run.lua:134`.