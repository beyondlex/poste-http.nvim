...
- 2026-06-29: indicators — spinner in sign column shows only one static frame, never animates. Root cause: `sign_define` updates the sign definition but already-placed signs keep their original text (baked in at `sign_place` time). Fix: in `update_spinner`, unplace + re-place the sign each frame to force a visual refresh. See `lua/poste/indicators.lua:279-293`.