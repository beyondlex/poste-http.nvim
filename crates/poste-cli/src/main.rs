...
fn handle_import(source: ImportSource) -> Result<()> {
    ...
    // Generate _index.http with import directives
    if !result.files.is_empty() {
        let mut index_content = String::new();
        index_content.push_str("# Generated by poste import\n");
        index_content.push_str("# Run: poste run ./_index.http --line <N>\n");
        index_content.push('\n');
        for http_file in &result.files {
            let file_stem = std::path::Path::new(&http_file.path)
                .file_stem()
                .and_then(|s| s.to_str())
                .unwrap_or("default");
            let alias = file_stem.replace('-', "_").replace(' ', "_");
            index_content.push_str(&format!("import ./{} as {}\n", http_file.path, alias));
        }
        let index_path = out_path.join("_index.http");
        std::fs::write(&index_path, &index_content)?;
        println!("  _index.http: {} import(s)", result.files.len());
    }
