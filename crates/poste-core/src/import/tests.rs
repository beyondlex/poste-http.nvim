...
fn test_empty_swagger_creates_no_files() {
    ...
    let result = importer.import(spec).unwrap();
    assert!(result.files.is_empty(), "empty swagger should produce no files");
}
...
fn test_empty_postman_creates_no_files() {
    ...
    let result = importer.import(spec).unwrap();
    assert!(result.files.is_empty(), "empty collection should produce no files");
}
...
fn test_import_result_serialization() {
    ...
    assert_eq!(decoded.env_vars.get("base_url").unwrap(), "https://example.com");
}
