    ...
    while i < n {
        ...
        match bytes[i] {
            ...
            b'$' if i + 1 < n && bytes[i + 1] == b'$' => {
                ...
                while i + 1 < n && !(bytes[i] == b'$' && bytes[i + 1] == b'$') { i += 1; }
                if i + 1 < n { i += 2; } else { i = n; }
                tokens.push(Token { kind: TokenKind::DollarStr, start, end: i });
            }
            ...
            b'a'..=b'z' | b'A'..=b'Z' | b'_' => {
                ...
                let kind = if is_known_keyword(word) { TokenKind::Keyword } else { TokenKind::Ident };
            ...
            b'!' if i + 1 < n && bytes[i + 1] == b'=' => {
                i += 2; tokens.push(Token { kind: TokenKind::Op, start, end: i });
            }
            b'<' | b'>' => {
                i += 1;
