...
- [ ] **P1a. `ContextType::String/Comment`** — 给 `ContextType` 枚举添加 `String` 和 `Comment` 变体。光标在字符串/注释内时返回对应类型，不再返回 `None`。
  - 文件: `context.rs`, `scanner.rs`, `detectors.rs`
  - 验收: `cargo test -p poste-core sql_context`

- [ ] **P1b. `version` 字段** — `detect_context()` 输出 JSON 添加 `"version": 1`。
  - 文件: `context.rs`
  - 验收: `cargo test -p poste-core sql_context`

- [ ] **P1c. `ctx_schema` 补全** — `SchemaTable` 上下文的 `ctx_schema` 填入 schema 名（当前为 null）。
  - 文件: `detectors.rs`
  - 验收: `cargo test -p poste-core sql_context`

- [ ] **P1d. `try_directive()` 降级** — 遇到 `@connection`/`@database` 标记时返回 `None`（安全网），不再返回 `Connection`。
  - 文件: `detectors.rs`
  - 验收: `cargo test -p poste-core sql_context`

### Lua 侧 (`lua/poste/sql/`)

- [ ] **P1e. 检测包装器** — 在 `completion.lua` 添加 `detect_context_for_completion(bufnr, line_before, cursor_line)`:
  - 保留 Lua 指令快速路径（`-- @connection`, `-- @database`）
  - SQL 正文优先调用 `try_rust_context()`（传完整正文，不预提取块）
  - 仅在 Rust 不可用时回退到 `completion_ctx.detect_context()`
  - `get_items()` 接入此新函数

- [ ] **P1f. 旧版开关** — 在 `completion.lua`:
...
- [ ] **P1h. `completion_ctx.lua` 标记废弃** — 添加头注释 `@deprecated` + "仅当 Rust 不可用时回退"。不再添加新 SQL 语法特性。
...
- [ ] **P2a. 定义 fixture 格式** — 使用 `█` 光标标记，JSON fixture 格式见 `README.md` §P2。放入 `tests/fixtures/sql_context/` 或内联在 Rust 测试中。
...
- [ ] **P2c. 测试运行器** — 添加 `crates/poste-core/tests/sql_context_golden.rs`。加载 fixture，去掉 `█`，调用 `detect_context()`，比较结果。
...
- [ ] **P3a. 新 `scope.rs` 模块** — 在 `crates/poste-core/src/sql_context/scope.rs`:
...
- [ ] **P3b. 兼容层** — `tables::extract_tables()` 内部调用 `scope::resolve_scope()`，保持返回 `Vec<TableRef>`。

- [ ] **P3c. 更新 `detect_context()`** — 每次调用只解析一次作用域，构建 `ContextResult`，移除重复 `extract_tables()` 调用。

- [ ] **P3d. 移除空行边界** — 从 `context.rs` 移除 `is_blank_line_separator()` 逻辑。`find_statement_token_range()` 只依赖 `;`。

- [ ] **P3e. 移除 `completion_ctx.lua` 启发式** — 删除 Lua SQL 启发式逻辑（非指令路径）。
