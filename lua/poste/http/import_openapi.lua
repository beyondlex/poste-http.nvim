...
local function do_import(spec_path, out_dir)
  ...
  local cmd = string.format("%s import openapi %s --out %s",
  ...
  vim.fn.jobstart(cmd, {
    ...
    on_stdout = function(_, data)
      ...
      for _, line in ipairs(data) do
        if line ~= "" then
          vim.notify(line, vim.log.levels.INFO, { title = "Import OpenAPI" })
        end
      end
    end,
    on_stderr = function(_, data)
      ...
      for _, line in ipairs(data) do
        if line ~= "" then
          vim.notify(line, vim.log.levels.WARN, { title = "Import OpenAPI" })
        end
      end
    end,
    on_exit = function(_, code)
      if code == 0 then
        vim.notify(string.format("OpenAPI import complete → %s", out_dir),
          vim.log.levels.INFO, { title = "Import OpenAPI" })
      else
        vim.notify("OpenAPI import failed", vim.log.levels.ERROR, { title = "Import OpenAPI" })
      end
    end,
  })
end
