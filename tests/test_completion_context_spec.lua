...
describe("completion context", function()
  ...
  describe("detect_context", function()
    ...
    it("returns 'file_directive' for single word at file level", function()
      local ctx, extra = context_detector.detect_context("GET", nil, nil, nil)
      assert.equals("file_directive", ctx)
      assert.is_nil(extra)
    end)

    it("returns 'method_or_header' for single word inside a block", function()
      local buf = vim.api.nvim_create_buf(false, true)
      vim.api.nvim_buf_set_lines(buf, 0, -1, false, { "### Test", "GET" })
      local ctx, extra = context_detector.detect_context("GET", buf, 2, 3)
